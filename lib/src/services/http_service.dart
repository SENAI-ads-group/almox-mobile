...
Future<http.Response> get(String url, {Map<String, String>? headers}) async => http
    .get(_parseUrl(url), headers: await _configureHeaders(headers))
...
Future<http.Response> post(String url, {Map<String, String>? headers, Object? body, Encoding? encoding}) async {
  return http
      .post(_parseUrl(url), headers: await _configureHeaders(headers), body: body, encoding: encoding)
      .timeout(const Duration(seconds: 5), onTimeout: () => http.Response('Error', 408));
}
